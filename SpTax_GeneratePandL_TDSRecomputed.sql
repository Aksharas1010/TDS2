alter Procedure [dbo].[SpTax_GeneratePandL_TDSRecomputed]                                          
(                                 
...
Begin                                                      
 Set Nocount On 
if exists(select distinct c.CLIENTID,s.SECURITY from Sauda s with(nolock) inner join Client c with(nolock) on c.clientid=s.clientid where   c.Type='NROCM' and CAST(s.TRANDATE as Date) between @StartDate and @ToDate and s.BuySell='S' and s.clientid=@clientid and s.SECURITY in (select distinct security from tax_Profit_Buynotfound_Manual where CAST(Trandate AS DATE) between @StartDate and @ToDate))
   begin
   print 'in'
...
    )
   truncate table #tempclients
   INSERT INTO #tempclients
   select distinct c.CLIENTID,s.SECURITY from Sauda s with(nolock) inner join Client c with(nolock) on c.clientid=s.clientid 
   where   c.Type='NROCM' and CAST(s.TRANDATE as Date) between @StartDate and @ToDate and s.BuySell='S' and s.clientid=@clientid 
   and s.SECURITY in (select distinct security from tax_Profit_Buynotfound_Manual where CAST(Trandate AS DATE) between @StartDate and @ToDate);

	--select * from #tempclients
	DECLARE  @cid int
    DECLARE @securitykey varchar(50)
	DECLARE ClientCursor CURSOR FOR
    SELECT clientid, securitykey FROM #tempclients
    OPEN ClientCursor
...
CREATE NONCLUSTERED INDEX idxdet1 ON [#SAUDA] ([BUYSELL],[SAUDATYPE],[DELSERVICEQTY]) INCLUDE ([TRANDATE],[SECURITY],[QTY],[RATE],[BROKERAGE],[SERVICETAX],[TOLEVY],[DelServiceTax],[DlvServiceChargePerQty],[derivedsduty])        
 ...
 Where   ClientId in ( @clientid )                                          
  And Trandate >= @FromDate                                          
  And  CAST(Trandate AS DATE) <= @ToDate 
 ...
 BalSaleValue- sum(SQSaleBrokerage/SQSaleQty+SQSaleServiceTax/SQSaleQty+SQSaleExchangeLevy/SQSaleQty+SQSaleStampDuty) *@balqty,1           
 from #sumtempbal t,security s(nolock)                              
 where t.security=s.security     
 ...
 BalSaleValue-sum(SQSaleBrokerage+SQSaleServiceTax+SQSaleExchangeLevy+SQSaleStampDuty)*BalSaleQty ,1              
 from #sumtempbal t,security s(nolock)                              
 where t.security=s.security   
  group by trandate,t.security,s.isinno,balsaleqty,BalSaleValue
...
-- FETCH NEXT FROM dateCursor INTO @ToDate; 
