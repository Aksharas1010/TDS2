alter Procedure SpGetTDSRecomputed_01
@frmdate DATE,
 @to_date DATE ,    
 @clientid int =null,  
 @FiscYear varchar(12)  
...
BEGIN  
 ...
 print 'JE'
 --------------------Journal Entry----------------------------------------------------------------------------------------------------
 declare @diffinsttax decimal(18,3)=(select sum(ST_Tax) from Client_Profit_Sums where TransSaleDate between @frmdate and @to_date and Client=@clientid)-(select sum(STTax) from TDS_JournalEntry where TransSaleDate between @frmdate and @to_date and Client=@clientid)
declare @diffinlttax decimal(18,3)=(select sum(LT_Tax) from Client_Profit_Sums where TransSaleDate between @frmdate and @to_date and Client=@clientid)-(select sum(LTTax) from TDS_JournalEntry where TransSaleDate between @frmdate and @to_date and Client=@clientid)
	 print @diffinsttax
	 print @diffinlttax
	...
	@to_date,  
    SUM(@diffinsttax+@diffinlttax),  
    CASE  
        WHEN CAST( @diffinsttax AS INT) != 0 AND CAST( @diffinlttax AS INT) != 0 THEN  
        ...
        WHEN CAST( @diffinlttax AS INT) != 0 and CAST( @diffinsttax AS INT)=0 THEN  
            'on Long term capital gain tax ' + CONVERT(VARCHAR(10), t.LT_TaxPercentage)  
        WHEN CAST(@diffinsttax AS INT) != 0 and CAST( @diffinlttax AS INT) = 0 THEN  
...
WHERE  
    t.Client = @clientid  
    AND t.TransSaleDate = (SELECT MAX(TransSaleDate) FROM Client_Profit_Sums)  
GROUP BY  
